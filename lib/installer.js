...
function acquireNJGitHub(version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let downloadUrl = util.format("https://smlnj.org/dist/working/%s/boot.amd64-unix.tgz", version);
        core.debug("Downloading SML/NJ from: " + downloadUrl);
        try {
            yield tc.downloadTool(downloadUrl, path.join("smlnj", "boot.amd64-unix.tgz"));
        }
        catch (error) {
        ...
        yield exec.exec("tree", [], { cwd: "smlnj" });
        console.log("ATTEMPT B:");
        yield exec.exec("tree", [], { cwd: path.join(process.cwd(), "smlnj") });
        yield exec.exec("build.sh", [], { cwd: path.join(process.cwd(), "smlnj") });
